...
public class PinYinUtil
{
     ...
     * @return
     */
    public static String getFirstSpell (String strChinese, boolean bUpCase)
    {
        try
        {
            ...
            for (int i = 0; i < b.length; i++)
            {
                if ((b[i] & 255) > 128)
                {
                    ...
                    buffer.append (getFirstSpellChar ((char) chart, bUpCase));
                    continue;
                }

                char c = (char) b[i];
                if (!Character.isJavaIdentifierPart (c))// 确定指定字符是否可以是
                                                        // Java,标识符中首字符以外的部分
                    c = '0';
        ...
        {
            LOGGER.warn ("\u53D6\u4E2D\u6587\u62FC\u97F3\u6709\u9519" + e.getMessage ());
        }
        return null;
    }
     ...
     * @return
     */
    private static char getFirstSpellChar (char strChinese, boolean bUpCase)
    {
