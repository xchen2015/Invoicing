...
<%@ page import="com.pinfly.purchasecharge.component.bean.TimeSpan"%>
	...
	<jsp:include page="outOrderAdd.jsp" />
	<jsp:include page="deliveryCompanyManagement.jsp" />
	<div style="width:100%; height:430px;">
		<table id="dg-outorder" title="<spring:message code="order.orderOutManagement" />" class="easyui-datagrid" toolbar="#toolbar-outorder" pagination="true" rownumbers="true" showFooter="true" 
			...
			data-options="onClickRow:onClickOutOrderRow, onLoadSuccess:onLoadOutOrderSuccess, onCheckAll:onCheckAllOutOrder, onUncheckAll:onUnCheckOutOrder, 
				onCheck:onCheckAllOutOrder, onUncheck:onUnCheckOutOrder,onBeforeLoad:onBeforeLoadOutOrder, onDblClickRow:onDblClickOutOrderRow">
			<thead>
				<tr>
					...
					<th field="statusCode" width="50" data-options="formatter:formatter_orderStatus, align:'center'"><spring:message code="order.statusCode" /></th>
					<th field="<%=PurchaseChargeConstants.CREATE_TIME%>" width="50" sortable="true"><spring:message code="order.createDate" /></th>
					<th field="<%=PurchaseChargeConstants.UPDATE_TIME%>" width="50" sortable="true"><spring:message code="order.updateDate" /></th>
<%-- 					<th field="<%=PurchaseChargeConstants.DEAL_MONEY %>" width="50" sortable="true"><spring:message code="order.inDealMoney" /></th> --%>
					<th field="<%=PurchaseChargeConstants.RECEIVABLE_MONEY%>" width="30" sortable="true"><spring:message code="order.inDealMoney" /></th>
					<th field="<%=PurchaseChargeConstants.PAID_MONEY %>" width="30" sortable="true" data-options=""><spring:message code="order.outPaidMoney" /></th>
					<th field="<%=PurchaseChargeConstants.PROFIT_MONEY%>" width="30" sortable="true" hidden="true"><spring:message code="order.profit" /></th>
					<%-- <th field="<%=PurchaseChargeConstants.PAY_TIME %>" width="50" sortable="true"><spring:message code="order.payDate" /></th> --%>
					<th field="userSignedTo" width="30" data-options="formatter:orderUserSignedFormatter"><spring:message code="order.signUserId" /></th>
					<th field="userCreated" width="30" sortable="true">操作员</th>
	...
	<div id="toolbar-outorder" style="padding:5px;height:auto">
		<div>
			<span id="div-outorder-typecode" style="margin-bottom:5px; margin-right:50px">
				<label><input id="typeCode_out_all" name="outorder_typeCode_mode" type="radio" value="0" checked="checked" onclick="onSearchOrderByAdvance()">所有</label>&nbsp;&nbsp;
				<label><input id="typeCode_out" name="outorder_typeCode_mode" type="radio" value="<%=OrderTypeCode.OUT%>" onclick="onSearchOrderByAdvance()">销售出货</label>&nbsp;&nbsp;
			...
			<span>
				<a href="javascript:void(0)" class="easyui-linkbutton c7" iconCls="icon-add" plain="true" 
					onclick="newOutOrder()" title="新增销售单">新增销售单</a> 
				<a id="btn-viewOutorder" href="javascript:void(0)" class="easyui-linkbutton" disabled="true" iconCls="iconSpan16 viewEye" plain="true" 
					onclick="viewOutOrder()" title="选择一行查看订单详情或勾选多行查看订单项">查看销售单</a> 
				<c:if test="${sessionScope.login_user.admin}">
					<a id="btn-deleteOutorder" href="javascript:void(0)" class="easyui-linkbutton" disabled="true" iconCls="icon-remove" plain="true" 
						onclick="deleteOutOrder()" title="选择一行删除订单">删除销售单</a>
				</c:if>
				<%-- <c:if test="${sessionScope.login_user.admin}">
					<a id="btn-manage-deliveryCompany" href="javascript:void(0)" class="easyui-linkbutton"  
						iconCls="icon-edit" plain="true" onclick="showManageDeliveryCompany()" title="">管理物流公司</a>
				</c:if> --%>
			</span>
		</div>
		<div>
	...
	<div id="dlg-outorder-preview" class="easyui-dialog"
		...
		<div style="width:100%; weight:100%;">
			<form id="fm-outorder" class="fm" style="margin:0 auto;padding:0px;width:650px;height:100%;" method="post" novalidate>
				...
				<div class="fitem css-dlg-formField">
					<label>所属工程:</label> 
					<input id="projectName" readonly="readonly" />
				</div>
	...
	<div id="dlg-buttons-outorder-preview">
		<a href="javascript:void(0)" id="submit-outorder-btn" class="easyui-linkbutton c7"
	...
	<div id="dlg-outorder-view" class="easyui-dialog"
		...
		<div class="ftitle" style="margin-left:30px; margin-right:30px;">出库单&nbsp;-&nbsp;<span id="order-id-span"></span></div>
		<div style="width:100%; weight:100%;">
			<form id="fm-outorder" class="fm" style="margin:0 auto;padding:0px;width:650px;height:100%;" method="post" novalidate>
				<div class="fitem divHidden">
					<input id="id" name="id" value="0">
	...
	<div id="dlg-multi-outorder-view" class="easyui-dialog"
		...
		<table id="dg-outorderItem-view" class="easyui-datagrid" style=""
	...
	<div id="dlg-buttons-multi-outorder-view">
		<a href="javascript:void(0)" class="easyui-linkbutton"
			iconCls="icon-cancel" onclick="javascript:$('#dlg-multi-outorder-view').dialog('close')"><spring:message code="close" /></a>
	</div>
	<!-- 查看多个订单 -->
			...
			<form id="fm-outorder" class="fm" method="post" style="margin:0 auto;padding:0px;width:100%;height:100%;" novalidate>
				<div class="fitem css-dlg-formField2">
					<div>
						...
						<input id="bid" name="bid" readonly="readonly" style="border: none;">
				...
				<div class="fitem css-dlg-formField2">
					<div>
						...
						<select id="orderStatus" name="statusCode" class="easyui-combobox" editable="false" panelHeight="auto" 
							 data-options="valueField:'value',textField:'text'" style="width:200px">
						</select>
					</div>
				</div>
				<div class="fitem css-dlg-formField2">
